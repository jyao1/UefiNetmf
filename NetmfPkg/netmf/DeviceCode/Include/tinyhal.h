...
extern void HAL_AssertEx();
        ...
        #define       ASSERT(i)  { if(!(i)) HAL_Assert(#i, __LINE__, __FILE__); }
        #define _SIDE_ASSERTE(i) { if(!(i)) HAL_Assert(#i, __LINE__, __FILE__); }
...
public:
    ...
    T* FirstValidNode() const { T* res = m_first; return res ? (res->Next() ? res : NULL) : NULL; }
    T* LastValidNode () const { T* res = m_last ; return res ? (res->Prev() ? res : NULL) : NULL; }
...
//
#if !(defined(UEFI) && defined(__GNUC__)) // JYAO1 - RISK
