...
extern ASSEMBLY_FILE_STRUCT mAssemblyFileStruct[];
    ...
    HRESULT Load()
    {
...
#if defined(PLATFORM_UEFI)
        {
          ...
          for (Index = 0; ; Index++) {
            ...
            CLR_Debug::Printf( "Load - %a\r\n", mAssemblyFileStruct[Index].Name );

            if ((mAssemblyFileStruct[Index].Start == NULL) ||
                (mAssemblyFileStruct[Index].End == NULL)) {
              continue;
            }

            TINYCLR_CHECK_HRESULT(LoadKnownAssemblies( mAssemblyFileStruct[Index].Start, mAssemblyFileStruct[Index].End ));
          }
        }
#else
