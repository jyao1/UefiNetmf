...
#if defined(PLATFORM_WINCE) || defined(PLATFORM_BLACKFIN) || defined(__GNUC__) || defined(__RENESAS__) || defined(UEFI)  // unfortunately WINCE doesn't support 1 byte alignment (even with _declspec unless the entire project is built with 1 byte alignment).
